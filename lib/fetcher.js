...
exports.getBuffer = getBuffer = async (url) => {
    ...
    const anu = fs.readFileSync('./tmp/emror.jpg')
...
exports.createExif = (pack, auth) =>{
    ...
    fs.writeFileSync('./tmp/data.exif', Buffer.concat([_, __, ___, ____]), function (err) {
        console.log(err)
        if (err) return console.error(err)
        return `./tmp/data.exif`
    })
...
try {
console.log(buff)
spawn('webpmux', ['-set','exif', './tmp/data.exif', buff, '-o', out])
.on('exit', () => {
