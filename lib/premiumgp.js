 ...
 * @param {object} _premi
 */
const addPremiumGp = (userId, data, hora, admin, expired, _premi) => {
    const obj = { id: userId, data: data, hora: hora, admin: admin, expired: Date.now() + toMs(expired) }
    _premi.push(obj)
    fs.writeFileSync('./grupo/premium.json', JSON.stringify(_premi))
}
...
const expiredCheck = (_dir) => {
    setInterval(() => {
        ...
        if (position !== null) {
            ...
            fs.writeFileSync('./grupo/premium.json', JSON.stringify(_dir))
        }
    }, 1000)
}
