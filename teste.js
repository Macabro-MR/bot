    ...
    conn.ev.on('messages.upsert', async (m) => {
        try {
            ...
            switch(comando) {
               ...
               /* case 'gen': {
                    const quantidade = 10; // Defina o número mínimo de cartões desejados
                    let cartoesGerados = [];

                    for (let i = 0; i < quantidade; i++) {
                        const numeroCartao = faker.random.number({ min: 4000, max: 4999 }) + ' ' +
                                        faker.random.number({ min: 1000, max: 9999 }) + ' ' +
                                        faker.random.number({ min: 1000, max: 9999 }) + ' ' +
                                        faker.random.number({ min: 1000, max: 9999 });
                        
                        const dataValidade = faker.date.future(5).toLocaleDateString();

                        const cvv = faker.random.number({ min: 100, max: 999 });
                    ...
                    // Envie os números de cartão gerados como uma mensagem
                    const mensagem = cartoesGerados.map((cartao, index) => {
                        return `Cartão ${index + 1}:\nNúmero: ${cartao.numeroCartao}\nData de Validade: ${cartao.dataValidade}\nCVV: ${cartao.cvv}\n`;
                    }).join('\n\n');

                    conn.sendMessage(from, { text: mensagem }, { quoted: info });
                }
                break */
